...
> (§8, spec §9 amendments large-null recalibration … increased coupling replication) recalibrated the detection threshold on a 600-run
> null, firmed the iso-line, and raised coverage-and-coupling campaign to ≥ 500 matched pairs. It **supersedes §3's
> lead times, §3.1's iso-line, §3.2's false-alarm rates, §4.1's timing contrast and §5's
> coverage-and-coupling campaign numbers.** §§2–6 are kept verbatim as the campaign-1 record — both operating points
...
| **Bifurcation validation** — bifurcation obligation | [`run_gate1_bifurcation.py`](run_gate1_bifurcation.py) | 832 equilibrium + 84 shock + 144 long runs | ~11 min |
| **coverage-and-noise campaign** headline sweep | [`run_ea_headline.py`](run_ea_headline.py) | 4 320 runs (4 c × 3 colours × 3 severities × 2 arms × 2 scenarios × 30 reps) | 1 199 s |
| **auxiliary arms** | [`run_aux_arms.py`](run_aux_arms.py) | 360 + 120 runs at (c = 0.5, mid red noise) | 145 s |
| **coverage-and-coupling campaign** joint sweep | [`run_eb.py`](run_eb.py) | 2 160 runs (3 c × 3 g × 2 modes × 120 reps) | 684 s |
...
## 2. BIFURCATION VALIDATION — bifurcation obligation: **PASS**
...
from the committed 700-cycle Bifurcation-validation rows. The corresponding 2,000-cycle cells give
...
control — is discharged by coverage-and-noise campaign's paired control arm (§3, FPR column), not here.

---

## 3. coverage-and-noise campaign — lead time under consent-bounded observation
...
difference in the table is smaller. **coverage-and-noise campaign does not resolve a non-random-consent penalty.**
...
## 5. coverage-and-coupling campaign — the joint (c × g) sweep
...
> check". The source defines **fourteen**, recomputation through aggregate verification. And "every number bound" (§1's
...
(recomputation–aggregate verification): a cell summary recomputed from the per-run rows in the same aggregate (recomputation, iso-line, aggregate verification,
remediation analysis, aggregate verification); a count or set property of seeds, e.g. no scored run on a calibration seed (seed hygiene, matched pairing);
byte identity of a rule file, e.g. the detection rule embedded in each aggregate against
`configs/detection_rule.json` (rule identity, aggregate verification, aggregate verification, both-class calibration); a stored flag compared to the stored numbers
it summarises (recomputation `meets_external_bar`, e-b decision rule `excludes_zero`, cross-campaign analysis `above_external_bar_ci_lower`,
remediation analysis the MNAR verdict string); every number in this file bound to an aggregate path through
...
interval or p-value anywhere — it calls `recal_analysis.compute()` (cross-campaign analysis) and
`remediation_analysis.compute()` (remediation analysis), and both of those modules compute Wilson intervals,
bootstrap quantiles and a heterogeneity p-value. What cross-campaign analysis and remediation analysis do with that output is diff it
...
`recal_analysis.json` that cross-campaign analysis reproduces by re-running that same module (self-reproduction, not
...
seven Bifurcation validation test-statistic claims. The verifier opens **13 of 104** aggregate files in
...
assertion evaluated across the fourteen classes recomputation–aggregate verification, so the same run can and does contribute
...
1. **C-III has no resolved evidence.** coverage-and-coupling campaign's coupling main effect and its (c × g)
   interaction are both null at 120 paired replicates, and the two g levels disagree in
   sign. Conflict 2 forbids a pointer-only coverage-and-coupling campaign while §1 carries C-III, so the paper as
   ...
   coupling strength"); (c) drop C-III and take the pointer-only coverage-and-coupling campaign that conflict 2 then
...
7. **Seed variation is large relative to several reported effects.** Δt s.e. is 9–16 cycles
   ...
   under-powered. Raising coverage-and-noise campaign to ~100 replicates costs about an hour and would firm up the
...
9. **Single substrate size for all reported experiments.** Bifurcation validation sweeps N to 1600, but
   coverage-and-noise campaign, coverage-and-coupling campaign and the auxiliaries are all at N = 400. The coverage floor's dependence on
...
coverage-and-coupling campaign run to ≥ 500 matched pairs, verdict by a pre-declared rule); item 2 → **§8.2–8.3**
...
*Three computation jobs, no design changes. Logged as spec §9 amendments **large-null recalibration**
(threshold recalibrated on a dedicated 600-run null), **coverage-grid refinement** (one added coverage-and-noise campaign grid point,
c = 0.333, at the headline severity, for iso-line localisation; replication raised to 100
seeds/cell on the iso-line neighbourhood) and **increased coupling replication** (coverage-and-coupling campaign raised to ≥ 500 matched pairs
...
| **coverage-and-noise campaign** re-evaluation + iso-line replication | [`run_ea_headline.py`](run_ea_headline.py) `--rule detection_rule_recal.json` | **7 200 runs** (5 c × 3 colours × 3 severities × 2 arms × 2 scenarios; 100 reps on c ∈ {0.25, 0.333, 0.5} at mid severity, 30 elsewhere) | 2 020 s |
| **auxiliary arms** re-evaluation | [`run_aux_arms.py`](run_aux_arms.py) `--rule …` | 360 + 120 runs, unchanged design | 625 s |
| **coverage-and-coupling campaign** at ≥ 500 matched pairs | [`run_eb.py`](run_eb.py) `--rule … --reps 500` | **9 000 runs** (3 c × 3 g × 2 modes × 500 reps) | 3 083 s |
...
Every coverage-and-noise campaign and auxiliary run in this campaign is the *same run* campaign 1 scored: a seed
...
runs overlap between the two coverage-and-noise campaign aggregates and **0** of them disagree on the seed.
...
3 cycles instead of 5 fires 14 cycles earlier at the median. That is the frozen matched false-alarm calibration
...
**4.39%** at the run unit, Wilson 95% CI **[3.77%, 5.11%]**; corrected to
**4.48%** at the (coverage, arm, replicate) group unit, Wilson 95% CI **[3.19%, 6.24%]**,
over the 720 groups (240 of size nine, 480 of size three) below — down from
campaign 1's **8.98%** [7.85%, 10.26%] on 2,160 controls. At the run unit the CI lower bound sat **above** both
...
> **CORRECTED 2026-09-08.** The correction above was itself wrong: it recomputed the interval
> over **400 independent control groups**, and 400 is not the true group count. Confirmed
> directly from `sim/aggregates/ea_headline_recal.json`'s `per_run_rows` filtered to
> `scen=="control"` (3,600 rows) and grouped by the (c, arm, rep) keys, with every group sharing
> one seed and 0 of 720 exceptions: the true structure is **720 groups** — 240 of size nine and
> 480 of size three (240×9 + 480×3 = 3,600). Recomputed at the correct 720-group unit by direct
> group collapse: the leading detector's point estimate is **4.48%** (32.222 of 720), Wilson 95%
> CI **[3.19%, 6.24%]**; the lagging detector — never printed at the group unit before this
> correction — is **4.86%** (35 of 720, exactly), Wilson 95% CI **[3.52%, 6.69%]**.
>
> **The leading collapse is approximate and the lagging collapse is exact, and that difference
> is structural rather than incidental.** The leading detector's outcome is invariant in 640 of
> the 720 groups and varies in the other 80, so its collapsed count, 32.222, sums fractional
> per-group outcomes rather than counting alarmed groups whole, and the Wilson interval on it
> inherits that approximation. The lagging detector is invariant in all 720 of 720 groups, so
> its 35-of-720 count and interval are exact.
>
> **The qualitative verdict does not weaken — it strengthens.** The true group-unit lower bound,
> **3.19%**, sits FURTHER below both the 3.5% declared target and the 3.6% external bar than the
> superseded **3.45%** did. The superseded **[3.45%, 5.56%]** figure traces to no committed
> computation in this repository. Two independent recomputations — one by a subagent, one by a
> separate model given only the raw data — agree exactly on the corrected figures above.
>
> **CORRECTED AGAIN 2026-09-10, AND THIS TIME THE POINT ESTIMATE SURVIVES WHILE THE INTERVAL DOES
> NOT.** The correction above is right that the unit is 720 groups and right that the leading
> detector's figure is 4.48%. It is wrong in two ways that a referee can check in one line.
>
> **A count of groups cannot be fractional.** "32.222 of 720" was printed as though it were a
> count. It is the SUM OF PER-GROUP ALARM FRACTIONS, which is a cluster mean, and the block above
> says so in its own next paragraph while the manuscript printed it as a count anyway.
>
> **A Wilson interval is not defined for a fractional success count.** The interval
> [3.19%, 6.24%] was computed as though 32.222 were a number of successes. Recomputed as a
> CLUSTER BOOTSTRAP resampling whole groups — 20,000 draws at seed 20260910, implementation at
> `sim/recompute_group_fpr.py` — the leading detector is **4.48% [3.47%, 5.56%]** and the lagging
> detector, computed the same way for comparability, is **4.86% [3.33%, 6.53%]**. The lagging
> detector's 35 of 720 remains a genuine integer count because it is invariant within every
> group; only the leading detector needed the cluster treatment.
>
> **THE VERDICT STILL HOLDS AND ITS MARGIN IS NOW THIN, WHICH IS THE HONEST READING.** The lower
> bound moves from 3.19% to **3.47%** against the 3.5% declared target and the 3.6% external bar.
> It is still below both, by 0.03 points rather than 0.31. The superseded block above says the
> corrected bound "sits FURTHER below" the target than the figure it replaced; on the correct
> interval that is no longer true, and this sentence supersedes it.
>
> **NO INTEGER COLLAPSE WAS USED, AND THE REASON IS MEASURED RATHER THAN ASSERTED.** A group is
> one seed across a 3x3 grid of noise colour and severity, not a set of replicates, so "at least
> one run in the group alarmed" is a union over nine counterfactual conditions rather than a
> false-alarm rate. That rule is contaminated by bundle size: at group size nine it reads 14.17%
> and at size three 10.00%, while the per-run rates behind them are 4.26% and 4.58%. It would
> also have moved the leading detector to 11.39% [9.27%, 13.92%], reversing the paper's central
> claim on a packaging artifact. The any, majority and all rules are printed by the script above
> so the choice is visible rather than asserted.
>
> **AND THE SUPERSEDED FIGURE MAY HAVE BEEN RIGHT ALL ALONG.** The block above retired a prior
> reading of **[3.45%, 5.56%]** on the ground that it "traces to no committed computation in this
> repository". The cluster bootstrap returns **[3.47%, 5.56%]** — the upper bound identical, the
> lower within 0.02. That is strong evidence the retired figure was a correct cluster interval
> and that the 2026-09-08 correction replaced it with an invalid one. It cannot be proved,
> because no seed or draw count was recorded for the retired figure, which is exactly why this
> correction ships with an implementation, a seed and a draw count.
...
### 8.4 coverage-and-noise campaign lead time under the recalibrated rule
...
A second effect in the same aggregate justifies coverage-grid refinement's added replication: on the 30 shared
...
spread of **0.33–0.41**. Every one of the nine sits inside the base variant’s own
bootstrap interval [0.315, 0.595], so comparator gain resolves no floor effect against
seed noise. Provenance: `comparator_gain_leg.rows` and
...
Same design, same seeds, same (c = 0.5, mid red noise) scope bound (fixed auxiliary-condition scope); only the scoring
...
### 8.8 coverage-and-coupling campaign at 500 matched pairs per cell — the C-III evidence gap
...
1. **The one interval that does exclude zero points the *opposite* way to the "gaming
   ...
   statistic. Whatever else coverage-and-coupling campaign shows, it does not show restricted observation protecting an
...
4. **The false-alarm rate still misses the external bar** — 4.48% [3.19, 6.24] against
   3.6% (§8.3's group-unit interval, corrected 2026-09-08 to the true 720-group structure) —
   and the paper must report the achieved rate, not the nominal one.
...
7. **What did not change:** Bifurcation validation (§2) is untouched — the bifurcation obligation is
...
rather than once per run. **The design effect is NOT the same for every detector, and treating
it as though it were is the defect corrected on 2026-09-10.** The three graph
observables return an identical fired/not-fired verdict across all nine noise-condition copies
in **494 of 494** groups, so their measured design effect is **9.00** exactly, which is what the
invariance predicts analytically. **The scalar comparator does not: it matches in 424 of 494
groups, 85.8%, and its measured design effect is 3.743**, printed below as 3.74. Every row in
the table below is now corrected at its OWN detector's effect. The previously published version
of this table divided every row by 9.00, which understated the comparator's chi-square by a
factor of 2.4 and is the reason the manuscript printed the comparator as coverage-independent.
...
**TWO findings survive, and this paragraph asserted the opposite until 2026-09-10.** Observed
susceptibility's false-alarm rate depends on observation coverage, chi-square 42.61 on 15
degrees of freedom, p = 0.00018. **So does the scalar comparator's, and marginally more
strongly: chi-square 44.01 on 15 degrees of freedom, p = 0.00011**, corroborated by a
permutation test at p = 0.0003. The raw per-cell rates show it without any statistics: the
comparator alarms at 14.70% at c = 0.0625 and at about 1 to 4% at high coverage. **THE
SUPERSEDED SENTENCE READ** "while the scalar comparator's does not, 18.30 on 15 degrees of
freedom, p = 0.247", and it was wrong because 164.73 was divided by 9.00 rather than by the
comparator's own measured 3.743. On the four-cell grid the same corrected test gives p = 0.058 and
...
that directly and now runs **3,696 checks, 0 failures** (classes aggregate verification–remediation analysis added; remediation analysis
...
| stage | script | scale | wall time |
|---|---|---|---|
| both-class threshold calibration (both-class sensitivity) | [`calibrate_class.py`](calibrate_class.py) | same 600-run null as large-null recalibration, same ≤ 3.5% target; only `leading.classes` and `leading.k` differ from the frozen rule | 412 s |
| remediation sweep (both-class sensitivity … system-size sensitivity) | [`run_remediation.py`](run_remediation.py) | **5,100 runs** — 14 variants (9 gain × 40 reps, driver/partial/size × 30 reps) × 5 c, degradation + control, every run scored under BOTH rules | 1,677 s |
| re-analysis (alarm-time estimator comparison … missingness-arm comparison) | [`remediation_analysis.py`](remediation_analysis.py) | no runs — pure function of `ea_headline_recal.json`, `ea_headline.json`, `remediation_runs.json`, `calibration_null_bothclass.json`, `gate1_bifurcation.json` | seconds |
...
### 9.3 The two-channel rule the manuscript described but never ran (both-class sensitivity)
...
place). both-class sensitivity calibrates the genuine both-class rule on the **same 600-run null** at the
...
### 9.4 Comparator gain (comparator-gain sensitivity) — the crux survives the sweep
...
### 9.5 Formation driver (degree-driven latency sensitivity) — the sensitivity check the code carried and never ran
...
### 9.6 Partial-observation amplitude (partial-observation-noise sensitivity) — the floor is one-sided in the assumed constant
...
### 9.7 System size (system-size sensitivity) — magnitude moves, the floor band does not
...
### 9.9 The MNAR sentence (missingness-arm comparison) — DELETE, now on an interval rather than a judgement
...
### 9.10 Terminology and the graph object (sliding-window graph definition)
...
6. **Abstract/§5 terminology corrected** per sliding-window graph definition; the verifier count in §5 updates from
   2,262 to **3,696** (§9.10, this campaign).
7. **What did not change:** Bifurcation validation, the red-noise kill of the advantage, the achieved-FPR
   obligation (§8.3), the revocation semantics leg, and coverage-and-coupling campaign's bounded-null reading are all
...
[`run_tranche2.py`](run_tranche2.py). Verifier class **aggregate verification** covers the aggregate;
...
and **+258.9 (N=800)** on the committed 700-cycle Bifurcation-validation rows, and **+119.9 / +134.0**
...
time, the middle band empties at both sizes. Bifurcation validation's bimodality conclusion stands;
...
3. **ramp_hold_recover is a hysteresis demonstration the paper gets for free**: the
   ...
   driver recovery is not system recovery, which is both a Bifurcation-validation corollary and a
...
3. ~~Bifurcation-validation text: add the mixture-BIC line~~; re-write the occupancy sentence as a
...
**Verifier coverage.** claim binding (§6) binds every `claims.json` entry to its aggregate path and
